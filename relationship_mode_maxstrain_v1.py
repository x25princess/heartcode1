# -*- coding: utf-8 -*-
# HEARTCODE Simulation â€” Relationship Mode v1.5: Physical Feedback Edition
...
def setup_logging():
    ...
    log_txt_path = os.path.join(log_dir, f"heartcode_relationshipstrain_log_{timestamp}.txt")
    log_csv_path = os.path.join(log_dir, f"heartcode_relationshipstrain_log_{timestamp}.csv")
    ...
    def interact(self, other, log_txt, csv_writer):
        ...
        if self.emotional_state == "awkward":
            ...
            if chaos < 0.2:
                ...
                spawn_threads()  # High emotion = thread storm
            log_event(self.name, message, self.emotional_state, self.trust, self.tension, log_txt, csv_writer)
