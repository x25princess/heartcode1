# -*- coding: utf-8 -*-
# Elenya Maternal Simulation v2.4 — "Unstable but Survivable"
# A version that allows more stress build-up, fewer recovery events, and faster emotional cycling
...
def setup_logging():
    ...
    log_txt_path = os.path.join(log_dir, f"heartcode_unstable_log_{timestamp}.txt")
    log_csv_path = os.path.join(log_dir, f"heartcode_unstable_log_{timestamp}.csv")
...
def lil_weirdo(elenya, queue):
    ...
    while elenya["child_alive"]:
        ...
        if random.random() < 0.03:
            ...
            queue.put("Background anxiety intensifies for no clear reason...")

        chaos = random.random()

        if chaos > 0.985:
            queue.put("Lil Weirdo softly reboots herself... trying to stay stable.")
            elenya["stress_level"] = max(0, elenya["stress_level"] - 2)

        elif chaos < 0.25 and (time.time() - last_major_event) > 4:
            last_major_event = time.time()
...
if __name__ == "__main__":
    ...
    queue.put("=== UNSTABLE BUT SURVIVABLE SESSION STARTED ===")
    queue.put("Elenya is initiating volatile child simulation...")
