# -*- coding: utf-8 -*-
# HEARTCODE Simulation â€” Relationship Mode v2.1: Persistent Strain Edition
# Includes persistent CPU strain, memory use, and periodic system stat logging
...
# === Persistent System Strain Functions ===
...
def spawn_memory_eaters():
    p = multiprocessing.Process(target=eat_memory_persistent)
    p.start()
    strain_processes.append(p)
...
def setup_logging():
    ...
    log_txt_path = os.path.join(log_dir, f"heartcode_relationshipstrain_persistent_log_{timestamp}.txt")
    log_csv_path = os.path.join(log_dir, f"heartcode_relationshipstrain_persistent_log_{timestamp}.csv")
